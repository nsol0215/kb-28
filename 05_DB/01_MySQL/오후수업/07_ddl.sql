...
insert into order_composite_pk values(3,1,10,now());
...
-- 컬럼추가
-- price, 정수타입(int), 기본값0, 코멘트는 '가격', null 허용 불가
alter table product
    add price int default 0 comment '가격' not null after name; -- 순서 [first] [after 기존컬럼]
...
desc product;
-- name에 not null 제약조건 변경
-- not null은 modify로 변경
alter table product
modify column name varchar(100 )not null;

-- 컬럼명 변경
-- mame을 prod_name으로 변경
alter table product
