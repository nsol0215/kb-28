...
public class MainService {
	...
	private void signUp() {
		...
		String id = null; // while문 안에 넣으면 while문 끝나고 사라져서 호출할 때 오류 남
		...
		for (Member member : members){
			if(member.getMemberId().equals(id)){//리스트에 중복 아이디가 있다면
		...
		if(isDuplicate){
			...
			break; // 중복이 아닐 때 while문 나가서 비번 입력
		}
		}
	...
	private void login() {
		...
		for(Member m : members){
			...
			if(foundmember != null){
			System.out.println(foundmember.getMemberName()+"님 환영합니다.");
